...
function buttonPlayerClicked() {
  this.id == "good" ? (changeBtnTo = "good") : (changeBtnTo = "bad");
  var playerName = this.parentNode.querySelector('[data-hook="name"').innerHTML;
  chrome.storage.local.set({ [playerName]: changeBtnTo });

  ChangeCSSButtonClicked(this, changeBtnTo);
}

function addButtonsToPlayer(player) {
  if (player.childNodes.length == 4) {
    return;
  }
  var playerName = player.querySelector('[data-hook="name"').innerHTML;
  var playerButton = document.createElement("button");
  playerButton.innerHTML = "&#9940";
  playerButton.id = "bad";
  playerButton.style.backgroundColor = "transparent";
  playerButton.addEventListener("click", buttonPlayerClicked);
  player.appendChild(playerButton);
  player.style.borderWidth = "4px";
  player.style.borderStyle = "solid";
  chrome.storage.local.get({ [playerName]: false }, function (result) {
    if (result[playerName] === "good") {
      ChangeCSSButtonClicked(player.querySelector("button"), "good");
    } else if (result[playerName] === "bad") {
      ChangeCSSButtonClicked(player.querySelector("button"), "bad");
    } else {
      player.style.borderColor = "transparent";
    }
  });
  /* chrome.storage.local.get([playerName]).then((result) => {
        if (result[playerName] == "good") {
            ChangeCSSButtonClicked(player.querySelector('button'), "good");
        } else if(result[playerName] == "bad") {
            ChangeCSSButtonClicked(player.querySelector('button'), "bad");
...
function ChangeCSSButtonClicked(clickedBtn, changeBtnTo) {
  if (changeBtnTo == "good") {
    innerText = "&#9989";
    playerColor = "green";
    idToChange = "bad";
  ...
  clickedBtn.id = idToChange;
  clickedBtn.innerHTML = innerText;
  clickedBtn.parentNode.style.borderColor = playerColor;
}
...
function changeView(viewIndex) {
  if (5 <= viewIndex <= 8) {
    ...
    gameframe.contentWindow.document
      .querySelector('[data-hook="settings"]')
      .click();
...
function record(gameview = true) {
  ...
  if (gameview) {
    gameframe.contentWindow.document
      .querySelector('[data-hook="menu"]')
      .click();
    var recBtn = waitForElement('[data-hook="rec-btn"]');
    recBtn.then(function (btn) {
      btn.click();
    });
    gameframe.contentWindow.document
      .querySelector('[data-hook="menu"]')
    ...
    gameframe.contentWindow.document
      .querySelector('[data-hook="rec-btn"]')
      .click();
  }
}

chatObserver = new MutationObserver(function (mutations) {
  var candidates = mutations
  ...
  var statSec =
    gameframe.contentWindow.document.getElementsByClassName("stats-view")[0];
  var chatInput = gameframe.contentWindow.document.querySelector(
    '[data-hook="input"]'
  );
  var chatLog =
    gameframe.contentWindow.document.querySelector('[data-hook="log"]');
  ...
  statSec.ondblclick = function () {
    var gameframe =
      document.documentElement.getElementsByClassName("gameframe")[0];
    var c =
      gameframe.contentWindow.document.getElementsByClassName("graph")[0]
        .firstChild;
    var ctx = c.getContext("2d");
    var imgData = ctx.getImageData(0, 63, 31, 1);
    var hexString = Array.prototype.map
      .call(new Uint8Array(imgData.data), (x) =>
    ...
    chatInput.value =
      statSec.innerText.replace(/\n/g, " ") +
      " Red bars: " +
      (hexString.match(/c13535ff/g) ? hexString.match(/c13535ff/g).length : 0);
    gameframe.contentWindow.document
      .querySelector('[data-hook="send"]')
      .click();
  };

  chatCheck = function (chatLine) {
    if (
      [...muted].filter((x) => chatLine.innerText.startsWith(x + ": ")).length >
      0
    ) {
      chatLine.hidden = true;
    } else if (
      muteAllToggle &&
      muteExceptions.filter((x) => chatLine.innerText.startsWith(x + ": ")) ==
        0 &&
    ...
    chatLine.oncontextmenu = function () {
      if (chatLine.innerText.includes(":")) {
        ...
        chatLine.className === "notice" &&
        chatLine.innerText.match(noticeRe)
      ) {
        var chatAuthor = chatLine.innerText
          .match(noticeRe)[0]
          .replace(" ", "_");
        if (chatInput.value !== null) {
          chatInput.value += " @" + chatAuthor + " ";
...
const emojiRe = new RegExp(
  "(" + RegExp.escape(Object.keys(emojiShortcuts).join("|")) + ")",
  "g"
);
    ...
    switch (true) {
      ...
      case tempView == "roomlist-view":
        ...
        chrome.storage.local.get(
          { haxSearchConfig: true, haxAutoJoinConfig: true },
          function (items) {
            if (items.haxSearchConfig) {
              createSearch();
            }
            if (items.haxAutoJoinConfig) {
              createButton();
            }
          }
        );
        ...
        var gameframe = document.getElementsByClassName("gameframe")[0];
        var changeNickBtn = gameframe.contentWindow.document.querySelector(
        ...
        addonSettingsBtn.onclick = function () {
          ...
          addonSettingsOpen.then(function (btn) {
            btn.click();
          });
        };
        ...
        sleep(500).then(function () {
          var gameDocument =
            document.getElementsByClassName("gameframe")[0].contentWindow
              .document;
          var players = gameDocument.getElementsByClassName("player-list-item");
        ...
        chrome.storage.local.get({ haxMuteConfig: true }, function (items) {
          if (items.haxMuteConfig) {
            ...
            muteAll.onclick = function () {
              if (muteAllToggle) {
                muteAllToggle = false;
                var chats = gameframe.contentWindow.document
                  .querySelector('[data-hook="log"]')
                  .getElementsByTagName("p");
                for (i = 0; i < chats.length; i++) {
                  chats[i].removeAttribute("hidden");
                }
                muteAll.innerText = "Mute";
              } else {
                muteAllToggle = true;
        ...
        chrome.storage.local.get(
          ...
          function (items) {
            if (items.haxShortcutConfig) {
              ...
              emojiDoc.innerText = "😊";
              emojiDoc.onclick = function () {
                chrome.runtime.sendMessage({ type: "emoji" });
              };

              chatInput.parentNode.insertBefore(
                emojiDoc,
                chatInput.parentNode.lastChild.previousSibling
              );
            }
          }
        );
      ...
      case Boolean(tempView.match(/^(room-view|player-list-item|notice)/)):
        // early exit
        if (Boolean(tempView.match(/^(room-view|player-list-item)/))) {
          sleep(500).then(function () {
            var gameDocument =
              document.getElementsByClassName("gameframe")[0].contentWindow
                .document;
        ...
        chrome.storage.local.get({ haxKickBanConfig: false }, function (items) {
          if (items.haxKickBanConfig) {
            var players = gameframe.contentWindow.document.querySelectorAll(
              "[class^=player-list-item]"
            );
            var adminStatus =
              gameframe.contentWindow.document.querySelector(
                "[class$='view admin']"
              ) !== null;
        ...
        chrome.storage.local.get({ haxNotifConfig: false }, function (items) {
          if (items.haxNotifConfig) {
            var notifOpt = {
              type: "basic",
              title: "Haxball All-in-one Tool",
              message: "You were moved into a team",
              iconUrl: "icon.png",
            };
            if (tempView.match(/^(player-list-item)/)) {
              playersMoved = mutations.filter(
                (x) =>
                  x.addedNodes.length > 0 &&
                  x.target.parentNode.className.match(/[blue|red]$/)
              );
              if (
                playersMoved
                  .flatMap((x) => Array.from(x.addedNodes))
                  .map((x) => x.childNodes[1].innerText)
                  .includes(myNick)
              ) {
            ...
            if (tempView == "notice") {
              var noticeMsgs = mutations
                .flatMap((x) => Array.from(x.addedNodes))
                .map((x) => x.innerText);
              if (
                noticeMsgs.filter((x) => x.startsWith(myNick + " was moved"))
                  .length > 0
              ) {
                chrome.runtime.sendMessage({ type: "team", opt: notifOpt });
              }
